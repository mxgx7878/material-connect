...
const AdminOrderItemEditModal: React.FC<AdminOrderItemEditModalProps> = ({
  ...
  // supplier_discount is PER UNIT — totals multiply by quantity.
  const previewNumbers = () => {
    const unitCost   = parseFloat(formData.supplier_unit_cost) || 0;
    const discountPU = parseFloat(formData.supplier_discount) || 0;
    const qty        = parseFloat(formData.quantity) || 1;

    const supplierGross    = unitCost * qty;
    const materialDiscount = discountPU * qty;
    const supplierNet      = Math.max(supplierGross - materialDiscount, 0);
    // Customer: 50% margin on FULL supplier price, THEN material discount
    const customerTotal    = Math.max(supplierGross * 1.5 - materialDiscount, 0);

    return { unitCost, discountPU, qty, supplierGross, materialDiscount, supplierNet, customerTotal };
  };
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs mt-1">
                Shown to the client as "Material Discount" — total = discount × quantity.
              </p>
            </div>
            ...
            {/* Total Preview */}
            {(() => {
              const p = previewNumbers();
              return (
                <div className="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-gray-300 rounded-xl p-4">
                  <h4 className="text-sm font-bold text-gray-900 mb-3">Pricing Preview</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        Supplier Cost × {p.qty}:
                      </span>
                      <span className="font-medium text-gray-900">${p.supplierGross.toFixed(2)}</span>
                    </div>
                    {p.materialDiscount > 0 && (
                      <div className="flex justify-between text-green-600">
                        <span>Material Discount (${p.discountPU.toFixed(2)} × {p.qty}):</span>
                        <span className="font-medium">-${p.materialDiscount.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between pt-2 border-t border-gray-300">
                      <span className="font-semibold text-gray-700">MC Pays Supplier (net):</span>
                      <span className="font-bold text-blue-600">${p.supplierNet.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t-2 border-gray-300">
                      <span className="font-bold text-gray-900">Customer Price (ex GST):</span>
                      <span className="font-bold text-lg text-green-700">${p.customerTotal.toFixed(2)}</span>
                    </div>
                    <p className="text-[11px] text-gray-400 pt-1">
                      Customer = (unit × 1.5 × qty) − material discount. Quoted items override this.
                    </p>
                  </div>
                </div>
              );

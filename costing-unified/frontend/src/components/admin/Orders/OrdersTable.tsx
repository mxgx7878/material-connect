 ...
 * - Consistent with UNIFIED costing (PricingService): customer item =
 *   (unit × 1.5 × qty) − (per-unit material discount × qty); delivery flat 10%
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-col gap-1.5">
                        <span
                          className={`inline-block px-2.5 py-1 text-xs font-semibold rounded-full border ${getOrderStatusBadgeClass(
                            order.order_status
                          )}`}
                        >
                      ...
                      <td className="px-4 py-4 text-right bg-blue-50/50">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="text-xs text-gray-600 space-y-0.5">
                            <div>Items: {formatCurrency((order as any).supplier_item_gross ?? order.supplier_item_cost)}</div>
                            {(((order as any).material_discount_total ?? (order as any).supplier_discount) ?? 0) > 0 && (
                              <div className="text-green-600">
                                Material Discount: -{formatCurrency((order as any).material_discount_total ?? (order as any).supplier_discount)}
                              </div>
                            )}

 ...
 *  Order total (EX GST)       = items + delivery − order discount + other charges
 *  GST                        = applied at INVOICE level only (InvoicePricingService),
 *                               on the invoice taxable amount AFTER the invoice discount.
...
class PricingService
{
    ...
    public static function orderTotals(Orders $order, bool $confirmedOnly = false): array
    {
        ...
        // GST is NOT applied at order level — it is calculated per invoice
        // (InvoicePricingService) on the invoice's taxable amount. Order totals
        // are therefore EX GST everywhere (listings, order views, headers).
        $gst              = 0.0;

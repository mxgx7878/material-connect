...
class MasterProductsController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $product->supplierOffers->each(function ($offer) {
            if ($offer->supplier && isset($offer->supplier->delivery_zones)) {
                ...
                if($offer->supplier->delivery_zones && is_string($offer->supplier->delivery_zones)) {
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'specifications'  => 'nullable|string',
            'unit_of_measure' => 'nullable|string|max:100',
            'tech_doc'        => 'nullable|string|url|max:1000',
            'photo'           => 'nullable|string|url|max:1000',
            'is_approved'     => 'boolean',
            'category'        => 'nullable|string|max:255|exists:category,id'
        ]);
        ...
        $approverId = $request->is_approved ? Auth::id() : null;
    ...
    public function update(Request $request, $id)
    {
        ...
        $product->update([
            ...
            'approved_by'     => $product->approved_by,
            'category'        => $request->category ?? $product->category,
            'tech_doc'        => $request->has('tech_doc') ? $request->tech_doc : $product->tech_doc,
            'photo'           => $request->has('photo') ? $request->photo : $product->photo,
        ]);

 ...
 *  - Products carry the CUSTOMER price only (lowest available offer × the
 *    PricingService item margin) as price_min / price_max. Raw supplier cost,
 *    supplier identity and offer data are never returned.
 *  - Service areas are coarsened to region labels only.
 *
 *  NOTE: routes/api.php previously re-registered `public/products` and
 *  `public/product-types` further down the file (OrderController /
 *  UserManagement). Laravel keeps the LAST registration, so this controller was
 *  never actually reached and raw supplier prices were served publicly. Those
...
class PublicController extends Controller
{
     ...
     * Same basis the portal's order wizard uses (lowest offer, 50% margin), but the
     * margin is applied HERE so the raw supplier price never leaves the server.
